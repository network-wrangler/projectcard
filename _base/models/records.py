...
from .types import AllOf, AnyOf, OneOf
...
class RecordModel(BaseModel):
    ...
    def check_one_of(cls, values):
        ...
        if sum([cls._check_field_exists(field, values) for field in cls.require_one_of]) == 1:
            return values
    ...
    def check_any_of(cls, values):
        ...
        if not cls.require_any_of:
            return values
        if any([cls._check_field_exists(field, values) for field in cls.require_one_of]):
            return values
...
class DBModel(BaseModel):
    ...
    def check_fks(cls, values):
        ...
        for table, field, fk_table, fk_field in cls.fk_fields:
            if not fk_table in values:
                log.warning(
                    f"Table {fk_table} for specified FK {table} not in {cls}. \
                            Proceeding without fk validation."
                )
                continue
            if not fk_field in values[fk_table]:
                raise ForeignKeyFieldMissing(f"Specified FK {fk_field} not in {fk_table}")
