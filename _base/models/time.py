...
# fixme
TimeString = Annotated[str, Field(description="A time string in the format HH:MM or HH:MM:SS")]
TimespanString = Annotated[List[TimeString], Field(min_length=2, max_length=2)]
...
def dt_overlaps(timespans: List[List[datetime]]) -> bool:
    ...
    for i in range(len(timespans)):
        for j in range(i + 1, len(timespans)):
            if (timespans[i][0] < timespans[j][1]) and (timespans[j][0] < timespans[i][1]):
                return True
    return False
...
def dt_overlap(timespans: List[List[datetime]]) -> bool:
    ...
    for i in range(len(timespans)):
        for j in range(i + 1, len(timespans)):
            if dt_overlaps(timespans[i][0], timespans[i][1], timespans[j][0], timespans[j][1]):
                return True
    return False
...
class Timespan:
    ...
    def __init__(self, value: list[TimeType]):
        ...
        Args:
            value (time): a list of two time strings, datetime objects, Time, or seconds from midnight.
        """
        if len(value) != 2:
            raise TimespanFormatError(
                "timespan must be a list of two time strings, datetime objects, Time, or seconds from midnight."
            )
