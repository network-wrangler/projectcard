...
FEED_CONFIG = {
    "read": ["frequencies", "routes", "shapes", "stop_times", "stops", "trips"],
    "hash": ["frequencies", "routes", "shapes", "stop_times", "stops", "trips"],
}
...
def file_from_feed(feed_path: Union[Path, str]) -> "Feed":
    ...
    Returns:
        Feed: A Feed object representing the GTFS feed.
    """
...
class Feed:
    ...
    def __init__(self, feed_path: Union[Path, str], config: dict = FEED_CONFIG):
        ...
        self._loc = _unpack_zip(feed_path)

        self._config = config

        for table in self.config["read"]:
            _table = table.replace(".txt", "")
            WranglerLogger.debug(f"...setting {_table}")
            setattr(self, table, self._read_from_file("table" + ".txt"))
    ...
    def validate_df_as_table(self, table: str, df: pd.DataFrame) -> bool:
        ...
        if self.SCHEMAS.get(table):
            ...
            WranglerLogger.debug(f"{table} requested but no schema available to validate.")
        return df
    ...
    def stops_node_id_from_stop_id(self, stop_id: Union[list[str], str]) -> Union[list[int], int]:
    ...
    def node_is_stop(
        ...
        args:
            ...
            pickup_type: str indicating logic for selecting stops based on piackup and dropoff
                ...
                "dropoff_only": only dropoff > 0
        """
        trip_stop_nodes = self.trip_stops(trip_id, pickup_type=pickup_type)[self.stops_node_id]
    ...
    def trip_stop_pattern(self, trip_id: str, pickup_type: str = "either") -> list[str]:
        ...
        args:
            ...
            pickup_type: str indicating logic for selecting stops based on piackup and dropoff
                ...
                "dropoff_only": only dropoff > 0
        """
        trip_stops = self.trip_stop_times_for_pickup_type(trip_id, pickup_type=pickup_type)
